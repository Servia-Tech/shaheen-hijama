...
const TOKEN = (fs.existsSync('token.txt') ? fs.readFileSync('token.txt', 'utf8').trim() : (process.env.WA_TOKEN || ''));
if (!TOKEN || TOKEN.length < 24) { console.error('SECURITY: write a 24+ char secret to token.txt (or set WA_TOKEN) before starting.'); process.exit(1); }
...
function tokenOf(req) {
  const h = req.headers.authorization || ''; if (h.startsWith('Bearer ')) return h.slice(7);
  const c = req.headers.cookie || ''; const m = c.match(/(?:^|;\s*)k=([^;]+)/); if (m) return decodeURIComponent(m[1]);
  return req.query.k || '';
}
function ok(req) { const t = tokenOf(req); return t.length === TOKEN.length && t === TOKEN; }
function deny(res) { res.status(401).send('unauthorized'); }
function setCookie(res) { res.setHeader('Set-Cookie', 'k=' + encodeURIComponent(TOKEN) + '; HttpOnly; SameSite=Strict; Path=/'); }

const app = express();
app.use(express.urlencoded({ extended: true }));
app.use(express.json());

app.get('/status', (req, res) => res.json({ connected, hasQr: !!currentQR, me, lastEvent })); // connection state only, no data
app.get('/qr.png', async (req, res) => { if (!ok(req)) return deny(res); if (connected || !currentQR) return res.status(204).end(); try { res.type('png').send(await qrcode.toBuffer(currentQR, { width: 340, margin: 2 })); } catch (e) { res.status(500).end(); } });
app.get('/messages', (req, res) => { if (!ok(req)) return deny(res); res.json(messages.slice(-200)); });
...
app.get('/', (req, res) => { if (!ok(req)) return deny(res); setCookie(res); res.set('Cache-Control', 'no-store'); res.send(QRPAGE); });
app.get('/ui', (req, res) => { if (!ok(req)) return deny(res); setCookie(res); res.set('Cache-Control', 'no-store'); res.send(UI); });
...
+ "async function load(){try{let ms=await (await fetch('/messages',{cache:'no-store',credentials:'same-origin'})).json();let f=document.getElementById('feed');let atBottom=f.scrollHeight-f.scrollTop-f.clientHeight<80;f.innerHTML=ms.map(m=>`<div class='m ${m.out?'out':''}'><div class='who'>${esc(m.out?'You':(m.name||m.from.split('@')[0]))}</div>${esc(m.text)}<div class='t'>${new Date(m.ts).toLocaleTimeString()}</div></div>`).join('');if(atBottom)f.scrollTop=f.scrollHeight;}catch(e){}}"
+ "async function send(){let to=document.getElementById('to').value.trim(),tx=document.getElementById('tx').value.trim();if(!to||!tx)return false;document.getElementById('tx').value='';await fetch('/send',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({to,text:tx})});setTimeout(load,400);return false;}"
+ "load();setInterval(load,3000);</script></body></html>";

app.listen(PORT, '127.0.0.1', () => console.log('WA bridge (secured) on http://127.0.0.1:' + PORT));
